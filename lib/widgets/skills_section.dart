...
class _SkillsSectionState extends State<SkillsSection> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _controller = AnimationController(
      vsync: this,
      duration: const Duration(seconds: 1000000), // Acts as a continuous timer
...
void _drawDashedLineAndArrow(Canvas canvas, Path path, double progress) {
  ...
  for (PathMetric metric in path.computeMetrics()) {
    // progress is elapsed time in seconds. Speed is 80 pixels/second.
    double distance = (progress * 80.0) % metric.length;
